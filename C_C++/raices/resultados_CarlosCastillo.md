...
R/: Significa que podemos establecer un intervalo $[a, b]$ dentro del cual se encuentra la raíz. Por ejemplo, si $f(a)<0$ "y"  $f(b)>0$ se puede decir que bajo las condiciones de continuidad, que existe al menos una raíz entre a y b. Este concepto es fundamental en métodos cerrados, como el método de bisección, porque estos métodos mantienen la raíz dentro de un intervalo durante el proceso iterativo.
...
En los resultados analizados, los valores iniciales utilizados fueron relativamente cercanos a las raíces, por lo que no se observa un caso de divergencia. Por ejemplo, Newton partiendo de $x_0 = 2.5$ alcanzó la raíz $2.1284190638$ en 5 iteraciones.
...
| Método | Raíz | Iteraciones | Observaciones |
|--------|------|-------------|---------------|
| Bisección | No Existe | 0 | No converge |
| Falsa Posición | No Existe | 0 | No converge |
| Brent | No Existe | 0 | No converge |
| Newton | 0.0000000000 | 9 | No converge |
| Secante | 1.5206782199 | 70 | converge pero es falso |
| Steffenson | 	-76.1593565394 | 9 | No converge |
...
| Falsa Posición | No Existe | 0 | No converge |
| Brent | No Existe | 0 | No converge |
| Newton | 0.0000000000 | 2 | No converge |
| Secante | 0.9788587397 | 41 | converge pero es falso |
| Steffenson | 	0.0000000000 | 2 | No converge |

**Métodos cerrados (bisección, falsa posición, Brent).** Los tres detectan la condición y se detienen antes de iterar, porque la verificación previa $𝑓(𝑎)⋅𝑓(𝑏)>0$ demuestra que no hay cambio de signo. El método reconoce que su hipótesis de aplicabilidad no se cumple y no entrega un resultado falso.
